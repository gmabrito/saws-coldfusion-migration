...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    azureOpenAI: process.env.AZURE_OPENAI_ENDPOINT ? 'configured' : 'not-configured (PoC stub)',
  });
});
...
app.listen(PORT, () => {
  console.log(`\n========================================`);
  console.log(`  SAWS AquaAI API Server`);
  console.log(`  Port: ${PORT}`);
  console.log(`  Env:  ${process.env.NODE_ENV || 'development'}`);
  console.log(`  OpenAI: ${process.env.AZURE_OPENAI_ENDPOINT ? 'configured' : 'stub mode'}`);
  console.log(`========================================\n`);
});

...
function getInitialUser() {
  const stored = localStorage.getItem('ceo_user');
  ...
  if (ssoToken) {
    ...
    if (decoded && decoded.email) {
      ...
      localStorage.setItem('ceo_token', ssoToken);
...
export function AuthProvider({ children }) {
  ...
  const login = (userData, token) => { localStorage.setItem('ceo_token', token); localStorage.setItem('ceo_user', JSON.stringify(userData)); setUser(userData); };
  const logout = () => { localStorage.removeItem('ceo_token'); localStorage.removeItem('ceo_user'); setUser(null); };

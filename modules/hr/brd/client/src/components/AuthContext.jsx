...
function getInitialUser() {
  ...
  if (ssoToken) {
    ...
    if (decoded && decoded.email) {
      ...
      localStorage.setItem('hr_token', ssoToken);
...
export function AuthProvider({ children }) {
  ...
  const login = (userData, token) => { localStorage.setItem('hr_token', token); localStorage.setItem('hr_user', JSON.stringify(userData)); setUser(userData); };
  const logout = () => { localStorage.removeItem('hr_token'); localStorage.removeItem('hr_user'); setUser(null); };

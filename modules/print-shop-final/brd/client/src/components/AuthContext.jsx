...
function getInitialUser() {
  ...
  if (ssoToken) {
    ...
    if (decoded && decoded.email) {
      ...
      localStorage.setItem('printshop_final_token', ssoToken);
...
export function AuthProvider({ children }) {
  ...
  const login = (userData, token) => { localStorage.setItem('printshop_final_token', token); localStorage.setItem('printshop_final_user', JSON.stringify(userData)); setUser(userData); };
  const logout = () => { localStorage.removeItem('printshop_final_token'); localStorage.removeItem('printshop_final_user'); setUser(null); };

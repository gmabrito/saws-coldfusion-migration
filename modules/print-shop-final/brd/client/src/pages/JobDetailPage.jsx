...
export default function JobDetailPage() {
  ...
  const handleApproval = async (approved) => {
    setUpdating(true);
    try {
      await jobService.approve(id, { approved });
      fetchJob();
    } catch (err) {
    ...
    <div className="page">
      ...
      {isAdmin && job.Status !== 'Completed' && job.Status !== 'Cancelled' && (
        <div style={{ marginTop: 20 }}>
          {job.RushOrder && job.Status === 'Submitted' && (
            <div style={{ marginBottom: 12, padding: 12, background: '#fff3cd', borderRadius: 4, border: '1px solid #ffc107' }}>
              <strong>Rush Order Approval Required</strong>
              <div style={{ marginTop: 8, display: 'flex', gap: 12 }}>
                <button className="btn btn-success btn-sm" onClick={() => handleApproval(true)} disabled={updating}>
                  Approve Rush
                </button>
                <button className="btn btn-danger btn-sm" onClick={() => handleApproval(false)} disabled={updating}>
                  Deny Rush
                </button>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            {job.Status === 'Submitted' && !job.RushOrder && (

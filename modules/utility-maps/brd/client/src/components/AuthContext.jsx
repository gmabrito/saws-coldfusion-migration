...
function getInitialUser() {
  ...
  if (ssoToken) {
    ...
    if (decoded && decoded.email) {
      ...
      localStorage.setItem('utilitymaps_token', ssoToken);
...
export function AuthProvider({ children }) {
  ...
  const login = (userData, token) => { localStorage.setItem('utilitymaps_token', token); localStorage.setItem('utilitymaps_user', JSON.stringify(userData)); setUser(userData); };
  const logout = () => { localStorage.removeItem('utilitymaps_token'); localStorage.removeItem('utilitymaps_user'); setUser(null); };
